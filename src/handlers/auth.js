...
export async function handleLogin(request, env) {
    try {
        ...
        let cookieString = `${COOKIE_NAME}=${sessionId}; Max-Age=${24 * 60 * 60}; Path=/; Domain=${new URL(request.url).hostname.split('.').slice(-2).join('.')}; HttpOnly; Secure; SameSite=Lax`;
        // 如果希望跨子域，Domain 可以设置为 .bob666.eu.org (假设这是你的主域)
